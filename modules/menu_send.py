...
def create_menu_welcome_image(user_name, user_avatar_url, user_cover_url, bot_avatar_url, menu_text):
    ...
    try:
        background = Image.open("zalosend.png").convert("RGB")
    ...
    draw_ribbon_trapezoid_label(base_img, text="SEND - MENU", font_path="font/ChivoMono-VariableFont_wght.ttf", font_size=32, fill_color=(255, 255, 0))
...
╔ 📢 𝘀𝗲𝗻𝗱.𝗔𝗹𝗹 — Gửi tin nhắn tới tất cả nhóm Zalo
╠ 📩 𝘀𝗲𝗻𝗱.𝗚𝗿𝗼𝘂𝗽𝗟𝗶𝗻𝗸 — Gửi tin nhắn qua link nhóm
╠ 📩 𝘀𝗲𝗻𝗱.𝗜𝗗𝗟𝗶𝘀𝘁 — Gửi tin nhắn theo danh sách ID nhóm
╠ 📬 𝗮𝘂𝘁𝗼.𝗟𝗶𝗻𝗸 — Tự động gửi danh sách link
╚ 🔗 𝘀𝗲𝗻𝗱.𝗟𝗶𝗻𝗸 — Gửi link đã tạo tới các nhóm

╔ 📩 𝘀𝗲𝗻𝗱.𝗦𝗺𝘀 — Gửi tin nhắn trực tiếp cho 1 người
╠ 📩 𝘀𝗲𝗻𝗱.𝗨𝘀𝗲𝗿 — Gửi nhiều tin nhắn cho 1 người
╠ 💾 𝘀𝗲𝗻𝗱.𝗖𝗼𝗱𝗲 — Gửi file code cho người được tag
╠ 🖼️ 𝘀𝗲𝗻𝗱.𝗣𝗶𝗰 — Gửi ảnh bằng link ảnh
╠ 🎨 𝘀𝗲𝗻𝗱.𝗦𝘁𝗸 — Gửi sticker cú đấm vào nhóm
╚ 📞 𝗰𝗮𝗹𝗹 — Gọi điện tới số điện thoại hoặc Zalo

➜ 🔗 𝗰𝗿𝗲𝗮𝘁𝗲𝗟𝗶𝗻𝗸 — Tạo link mời tham gia tuỳ chỉnh
"""
