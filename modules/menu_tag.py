...
def create_menu_welcome_image(user_name, user_avatar_url, user_cover_url, bot_avatar_url, menu_text):
    ...
    try:
        background = Image.open("zalotag.jpg").convert("RGB")
        background = background.resize((WIDTH, HEIGHT), Image.LANCZOS)
    except Exception as e:
        logging.error(f"Lỗi khi mở ảnh zalotag.png: {e}")
    ...
    draw_ribbon_trapezoid_label(base_img, text="TAG - MENU", font_path="font/ChivoMono-VariableFont_wght.ttf", font_size=32, fill_color=(255, 255, 0))
...
def handle_menu_command(message, message_object, thread_id, thread_type, author_id, client):
    ...
    menu_message = """
════ 🇺 🇸 🇪 🇷 ════
➜ 🔔 𝘁𝗮𝗴.𝗔𝗹𝗹 — Gắn thẻ tất cả thành viên trong nhóm
➜ 🔔 𝘁𝗮𝗴.𝗔𝗹𝗹𝗠𝘀𝗴 — Gắn thẻ tất cả kèm nội dung tùy chỉnh
➜ 🔔 𝘁𝗮𝗴.𝗠𝗲𝗺 — Gắn thẻ một thành viên cụ thể
══════════════════════
"""
