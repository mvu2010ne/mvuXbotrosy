...
def handle_down_command(message, message_object, thread_id, thread_type, author_id, client):
    ...
    retry_strategy = Retry(
        total=5,  # Tăng số lần thử lên 5
        backoff_factor=2,  # Tăng thời gian chờ giữa các lần thử
        status_forcelist=[500, 502, 503, 504, 429],
        allowed_methods=["HEAD", "GET", "POST"]  # Đảm bảo retry cho các phương thức này
    )
    ...
    def upload_to_catbox(file_path):
        ...
        for attempt in range(3):
            try:
                ...
                response = session.post(url, data=data, files=files, timeout=60)  # Tăng timeout
    ...
    def download_image(link):
        try:
            response = session.get(link, headers=headers, timeout=60)  # Tăng timeout
            response.raise_for_status()
            ...
            raise Exception(f"Lỗi kết nối khi tải ảnh: {str(e)}")
        except RequestException as e:
            raise Exception(f"Lỗi khác khi tải ảnh: {str(e)}")

    def download_direct_video(link):
        try:
            response = session.get(link, headers=headers, stream=True, timeout=60)  # Tăng timeout
            response.raise_for_status()
            content_type = response.headers.get('content-type', '')
            ...
            raise Exception(f"Lỗi kết nối khi tải video trực tiếp: {str(e)}")
        except RequestException as e:
            raise Exception(f"Lỗi khác khi tải video trực tiếp: {str(e)}")
