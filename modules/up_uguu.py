...
des = {
    'tác giả': "Minh Vũ",
    'mô tả': "📤 Tải video từ tin nhắn reply lên Uguu.se.",
    'tính năng': [
        "📹 Nhận video từ tin nhắn reply và tải về máy chủ.",
        "📤 Tải video lên Uguu.se và trả về link tải.",
        "🗑️ Dọn dẹp file tạm sau khi xử lý.",
        "⚠️ Thông báo lỗi nếu video không hợp lệ hoặc tải lên thất bại."
    ],
    'hướng dẫn sử dụng': [
        "📩 Reply vào một video và gửi lệnh up.uguuse.",
        "📌 Ví dụ: reply video rồi gửi up.uguuse",
        "✅ Nhận link video trên Uguu.se trong tin nhắn."
    ]
}
...
# Hàm tải file lên Uguu.se
def upload_to_uguuse(file_path, content_type='video/mp4'):
    try:
...
def handle_uploadvideo_command(message, message_object, thread_id, thread_type, author_id, client):
    ...
    try:
        ...
        # Tải lên Uguu.se
        video_url = upload_to_uguuse(video_path, content_type='video/mp4')
        if video_url:
            # Gửi link
            client.sendMessage(Message(text=f"{video_url}"),
                               thread_id=thread_id, thread_type=thread_type, ttl=60000)
        else:
            raise Exception("Không thể tải video lên Uguu.se.")
...
def get_mitaizl():
    return {
        'up.uguuse': handle_uploadvideo_command
    }