...
# Danh sách các header để xoay vòng (mở rộng)
DANH_SACH_HEADER = [
...
# Hàm gọi API để tạo ảnh bìa V2
def lay_anh_bia_v2(ten, id, subname):
    # URL API với các tham số
    url = f"https://api-dowig.onrender.com/fbcover/v2?name={ten}&id={id}&subname={subname}"
    ...
    try:
        ...
        if phan_hoi.status_code == 200:
            ...
            duong_dan_anh = os.path.join(THU_MUC_CACHE, "canvasv2.png")
...
# Hàm xử lý lệnh *canvasv2 và gửi ảnh
def xu_ly_lenh_canvasv2(noi_dung_tin_nhan, doi_tuong_tin_nhan, thread_id, thread_type, author_id, client):
    print(f"Nhận lệnh: {noi_dung_tin_nhan}")
    
    

    # Tách tham số sau *canvasv2
    tham_so = noi_dung_tin_nhan[9:].strip()  # Bỏ *canvasv2 (9 ký tự)
    noi_dung = tham_so.split('|')

    # Kiểm tra xem có đủ tham số không
    if len(noi_dung) < 3:
        thong_bao_loi = Message(text="❌ Vui lòng nhập đầy đủ: tên|ID|subname.")
    ...
    try:
        ...
        id = noi_dung[1].strip()
        subname = noi_dung[2].strip()
        print(f"Tham số: tên={ten}, id={id}, subname={subname}")
    except Exception as e:
    ...
    thong_bao_dang_tai = Message(text="✨ Đang tạo ảnh bìa V2... Vui lòng đợi một chút.")
    ...
    duong_dan_anh = lay_anh_bia_v2(ten, id, subname)
    ...
    if duong_dan_anh:
        ...
        thong_bao_thanh_cong = Message(text=f"✨ Đã tạo ảnh bìa V2 thành công! Hãy lưu ảnh để xem full kích cỡ{kich_thuoc}:")
        try:
        ...
        thong_bao_loi = Message(text="❌ Đã xảy ra lỗi khi tạo ảnh bìa V2. Vui lòng thử lại.")
        client.replyMessage(thong_bao_loi, doi_tuong_tin_nhan, thread_id, thread_type, ttl=60000)
