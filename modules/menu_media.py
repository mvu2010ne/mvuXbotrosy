...
def create_menu_welcome_image(user_name, user_avatar_url, user_cover_url, bot_avatar_url, menu_text):
    ...
    draw_ribbon_trapezoid_label(base_img, text="MEDIA - MENU", font_path="font/ChivoMono-VariableFont_wght.ttf", font_size=32, fill_color=(255, 255, 0))
...
╔ 🖼️ 𝐀̉𝐍𝐇
╠ 📝 𝗯𝘀𝘁 — Ảnh từ bộ sưu tập (json)
╠ 📜 𝗮𝗱𝗱𝗕𝘀𝘁 — Thêm ảnh vào bộ sưu tập
╠ 📜 𝗱𝗲𝗹𝗯𝘀𝘁 — Xóa ảnh khỏi bộ sưu tập
╠ 🔧 𝗳𝗶𝘅 - Sửa tên bộ sưu tập
╠ 🍥 𝗻𝗲𝗸𝗼 — Ảnh neko
╠ 🌸 𝗴𝗮𝗶 — Ảnh gái xinh (girl1.txt)
╠ 😂 𝗵𝗮𝗵𝗮 — Ảnh meme (haha.txt)
╠ 🌟 𝐉𝐩 — Ảnh idol Nhật (jav.txt)
╠ 😸 𝐌𝐥𝐞𝐦 — Ảnh mông (mlem.txt)
╠ 🎌 𝐎𝐭𝐚𝐤𝐮 — Ảnh anime (otaku.txt)
╚ 🎌 𝐩𝐨𝐧𝐠𝐢𝐟 — Porn gif (otaku.txt)

╔ 📽️ 𝐕𝐈𝐃𝐄𝐎
╠ 🔞 𝐠𝐚𝐢𝘁𝘁 — Video gái nhảy tiktok(json)
╠ 🔥 𝘃𝐝𝟏𝟖 — Video 18+ (vd18.txt)
╠ 🌸 𝘃𝗱𝗴𝗮𝗶 — Video từ json Github
╚ 🔞 𝐕𝐝𝐱 — Video người lớn (vdx.txt)
══════════════════════
"""
...
def get_mitaizl():
    ...
    """
    return {
        'menu.media': handle_menu_command
    }