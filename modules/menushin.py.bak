...
def load_menu_data():
    ...
    try:
        ...
        return {
            "menu": {
                "commands": ["Lỗi: Không đọc được menu!"],
                "total": 0
            }
        }
...
def handle_menu_command(message, message_object, thread_id, thread_type, author_id, client):
    """
    Xử lý lệnh menu và các lệnh phụ:
      - Gửi phản hồi bằng emoji "✅" để xác nhận lệnh.
      - Đọc dữ liệu menu từ tệp JSON.
      - Tạo ảnh menu động và gửi kèm tin nhắn.
    """
    action = " ".join(message.lower().strip().split())  # Chuẩn hóa lệnh
    menu_data = load_menu_data()
    
    # Mặc định là menu chính
    menu_key = "menu"
    menu_name = "tổng hợp"
    total_commands = menu_data.get("menu", {}).get("total", 0)
    menu_commands = menu_data.get("menu", {}).get("commands", ["Không có lệnh!"])
    
    # Danh sách ánh xạ lệnh phụ với khóa JSON
    menu_subcommands = {
        "menu auto": "auto",
        "menu bot": "bot",
        "menu cmd": "cmd",
        "menu code": "code",
        "menu checklink": "checklink",
        "menu file": "file",
        "menu fun": "fun",
        "menu game": "game",
        "menu group": "group",
    ...
    # Kiểm tra các lệnh phụ
    for command, key in menu_subcommands.items():
        if action == command:
            menu_key = key
            menu_name = key
            total_commands = menu_data.get(key, {}).get("total", 0)
            menu_commands = menu_data.get(key, {}).get("commands", ["Không có lệnh!"])
            break
    ...
    try:
        image_path = create_menu_welcome_image(
            user_name, user_avatar_url, user_cover_url, bot_avatar_url, 
            menu_message, menu_name, total_commands
        )
    except:
        image_path = None
