...
def draw_gradient_text(draw, text, position, font, gradient_colors, shadow_offset=(4, 4)):
    """
    Vẽ text với hiệu ứng gradient kết hợp bóng chữ màu đen.
    Bóng chữ được in ra với offset (4,4) giống như dòng tên người dùng.
    """
    if not text:
        return
    text_len = len(text)
...
def create_menu_welcome_image(user_name, user_avatar_url, user_cover_url, bot_avatar_url, menu_text):
    ...
    Tham số:
      ...
      - user_avatar_url: URL avatar của người dùng.
      - user_cover_url: URL ảnh bìa của người dùng (nếu có).
      - bot_avatar_url: URL avatar của chính bot.
      - menu_text: Nội dung tin nhắn menu (danh sách lệnh, hướng dẫn, ...).
      
    Trả về:
      - Đường dẫn file ảnh được tạo ("menu_welcome.jpg").
    """
    ...
    x_name = rect_x0 + (rect_x1 - rect_x0 - name_w) // 2
    y_name = rect_y0 + padding_top
    # Chọn ngẫu nhiên 1 dải màu cho tên người dùng
    gradient_for_name = MULTICOLOR_GRADIENT
    draw_gradient_text(base_draw, name_text, (x_name, y_name), font_title, gradient_for_name, shadow_offset=(4,4))
    ...
    custom_texts = [
        {
            ...
            "font": get_font("font/Tapestry-Regular.ttf", 60)
        },
        {
            "text": "TRÒ CHƠI ",
            "font": get_font("font/ChivoMono-VariableFont_wght.ttf", 50)
        },
        {
            "text": "Xin mời chọn lệnh",
            "font": get_font("font/ChivoMono-VariableFont_wght.ttf", 45)
        },
        {
    ...
    for i, item in enumerate(custom_texts):
        text = item["text"]
        font_custom = item["font"]
        bbox_text = base_draw.textbbox((0, 0), text, font=font_custom)
        text_w = bbox_text[2] - bbox_text[0]
        x_text = rect_x0 + (rect_x1 - rect_x0 - text_w) // 2

        # Vẽ text với hiệu ứng gradient và bóng chữ với shadow_offset=(4,4)
        draw_gradient_text(base_draw, text, (x_text, current_y), font_custom, gradients_for_text[i], shadow_offset=(4,4))
        current_y += line_heights[i] + spacing
    draw_ribbon_trapezoid_label(base_img, text="GAME - MENU", font_path="font/ChivoMono-VariableFont_wght.ttf", font_size=32, fill_color=(255, 255, 0))
...
═══ 🇬 🇦 🇲 🇪 ═══
   ...
   ➜ 🎲 𝐁𝐜𝐮𝐚 — Chơi trò Bầu Cua vui nhộn  
   ➜ 🎲 𝐓𝐱𝐢𝐮 — Chơi Tài Xỉu với kết quả ngẫu nhiên  
   ➜ 🎲 𝐓𝐚𝐢𝐱𝐢𝐮 — Chơi Tài Xỉu (phiên bản khác)  
   ➜ 🧠 𝐒𝐨𝐢 — Soi cầu dự đoán kết quả Tài Xỉu  
   ➜ 🖼️ 𝐃𝐡𝐛𝐜 — Chơi Đuổi hình bắt chữ với 1 ảnh  
   ➜ 🖼️ 𝐃𝐡𝐛𝐜𝟐 — Chơi Đuổi hình bắt chữ với 2 ảnh  
   ➜ 🎯 𝐑𝐚𝐧𝐝𝐨𝐦 — Trò chơi ngẫu nhiên đầy bất ngờ  
   ➜ 🔢 𝐕𝐭𝐯 — Tham gia Vua Tiếng Việt giải đố  
   ➜ 🎲 𝐍𝐠𝐚𝐮𝐧𝐡𝐢𝐞𝐧 — Random kết quả ngẫu nhiên   
   ➜ 📡 𝐍𝐭 — Tham gia trò chơi nối từ  
   ➜ 📡 𝐜𝐨𝐭𝐮𝐨𝐧𝐠 — Chơi cờ tướng 


═══ ☀ 𝐌𝐄𝐍𝐔 𝐆𝐀𝐌𝐄 ═══

   ➜ 📜 𝐁𝐜 — Xem menu chi tiết của trò Bầu Cua  
   ➜ 📜 𝐓𝐱 — Xem menu chi tiết của trò Tài Xỉu  
"""
    # Gửi phản hồi emoji "✅" xác nhận đã nhận lệnh
    action = "✅ "
    client.sendReaction(message_object, action, thread_id, thread_type, reactionType=75)

    # Lấy thông tin người dùng từ author_id (nếu có hàm fetchUserInfo)
    try:
    ...
    try:
        ...
        print(f"[DEBUG] Bot avatar URL: {bot_avatar_url}")
        logging.info(f"[DEBUG] Bot avatar URL: {bot_avatar_url}")
    ...
    client.sendLocalImage(
        ...
        ttl=30000,  # thời gian tồn tại của tin nhắn (ms)
        width=1472,
        height=600
    )
...
def get_mitaizl():
    ...
    Trong đó:
      - 'menu1': Tham chiếu đến hàm handle_menu_command.
    """
    return {
        'menu.game': handle_menu_command
    }
